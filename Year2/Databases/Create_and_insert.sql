---RE-STRUCTURE THE DATABASE ->LEAGUE =COMPETITION ->CHANGE IT TO "FEDERATION"
use master;
go
CREATE TABLE Country(
Pk_CountryId int IDENTITY(1,1) PRIMARY KEY,
CountryName varchar(100)
);
go
CREATE TABLE Team(
Pk_TeamId int IDENTITY(1,1) PRIMARY KEY,
TeamName varchar(100),
);
CREATE TABLE Player(
Pk_PlayerId int IDENTITY(1,1) PRIMARY KEY,
PlayerName varchar(100),
NrOnShirt int,
Fk_CountryId int FOREIGN KEY REFERENCES Country(Pk_CountryId),
Fk_TeamId int FOREIGN KEY REFERENCES Team(Pk_TeamId)
);

CREATE TABLE PlayerStats(
Pk_PlayerStatsId int IDENTITY(1,1) PRIMARY KEY,
NrOfGoals int,
Appearances int,
Fk_PlayerId int UNIQUE FOREIGN KEY REFERENCES Player(Pk_PlayerId) ON DELETE CASCADE  ON UPDATE CASCADE
);

CREATE TABLE Fixtures(
Pk_FixturesId int IDENTITY(1,1) PRIMARY KEY,
NrOfMatches int ,
Fk_TeamId int UNIQUE FOREIGN KEY REFERENCES Team(Pk_TeamId) ON DELETE CASCADE  ON UPDATE CASCADE

);


CREATE TABLE Coach(
Pk_CoachId int IDENTITY(1,1) PRIMARY KEY,
...
Pk_CoachStatsId int IDENTITY(1,1) PRIMARY KEY,
NrOfChampionshipsWon int,
NrOfMatchesWon int,
Fk_CoachId int UNIQUE FOREIGN KEY REFERENCES Coach(Pk_CoachId) ON DELETE CASCADE ON UPDATE CASCADE
);

CREATE TABLE Season(
	Pk_SeasonId int IDENTITY(1,1) PRIMARY KEY, 
	SeasonYear date
);

CREATE TABLE Federation(
	Pk_FederationId int IDENTITY(1,1) PRIMARY KEY,
	FederationName varchar(100) UNIQUE,
	Fk_CountryId int FOREIGN KEY REFERENCES Country(Pk_CountryId),
	
);
CREATE TABLE Competition(
Pk_CompetitionId int IDENTITY(1,1) PRIMARY KEY,
CompetitionName varchar(100),
Fk_FederationId int FOREIGN KEY REFERENCES Federation(Pk_FederationId)

);
CREATE TABLE SeasonsCompetitions(
Fk_SeasonId int  FOREIGN KEY REFERENCES Season(Pk_SeasonId),
Fk_CompetitionID int FOREIGN KEY REFERENCES Competition(Pk_CompetitionId),
PRIMARY KEY(Fk_SeasonId,Fk_CompetitionId)
);
CREATE TABLE TeamsCompetitions(
	Fk_TeamId int  FOREIGN KEY REFERENCES  Team(Pk_TeamId),
	Fk_CompetitionId int FOREIGN KEY REFERENCES Competition(Pk_CompetitionId),  
	PRIMARY KEY(Fk_TeamId, Fk_CompetitionId)
);
...
set identity_insert  PlayerStats ON
set identity_insert   PlayerStats OFF
...
set identity_insert CoachStats ON
set identity_insert   CoachStats OFF
